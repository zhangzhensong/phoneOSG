...
{
	//fstream binary_file("F:/out.jpg",ios::out|ios::binary|ios::app); 
	fstream binary_file(outfile, ios::out|ios::binary|ios::app); 
	binary_file.write(out,count);
	binary_file.close();
} 
...
void read_from_binary_file(char * in, int & count, const char* infile)
{
	char ch;
	count = 0;
	//fstream binary_file("F:/images.jpg",ios::binary|ios::in);
	fstream binary_file(infile, ios::binary|ios::in);
	char *p = in;

	while(!binary_file.eof())
	{
		binary_file.read(&ch,sizeof(char));
		//cout << ch;
		//in.push_back(ch);
		*p++ = ch;
		count++;
...
//	string strJpg;
//
//	char * in = new char[159*318];
//	int count;
//	read_from_binary_file(in, count);
//	
//	char * tmp = new char[159*318];
//	int tmplen = 159*318;
//	memset(tmp, 0, tmplen);
//	base64_encode(in, count, tmp, tmplen);
//
//	char * out = new char[159*318];
//	int outlen = 159*318;
//	base64_decode(tmp, strlen(tmp), out, &outlen);
//
//
//	write_to_binary_file(out, outlen );
//
